...
export default function OnboardingPage() {
  ...
  return (
    <div className="min-h-screen bg-[#0B1120] flex flex-col items-center py-6 px-4 overflow-y-auto">
      <div className="max-w-3xl w-full flex flex-col flex-1 pb-10">
        ...
        <div className="flex flex-col items-center text-center mb-6 shrink-0">
          ...
          <h1 className="text-3xl font-bold tracking-tight text-white mb-1">Select Repositories</h1>
          <p className="text-sm text-muted-foreground">
            Choose the GitHub repositories you want Syntrix to analyze and index.
          </p>
        </div>

        <div className="bg-surface border border-white/10 rounded-2xl flex flex-col overflow-hidden max-h-[calc(100vh-220px)] min-h-[450px] shadow-2xl">
              ...
              <div className="space-y-1.5 p-2">
                {filteredRepos.map(repo => (
                  ...
                  >
                    ...
                    {repo.language && (
                      <span className="text-[10px] text-muted-foreground px-2 py-1 rounded-full bg-white/5 whitespace-nowrap border border-white/5">
                        {repo.language}
                      </span>
                    )}
                  </div>
          ...
          <div className="p-4 border-t border-white/10 bg-black/40 flex justify-between items-center shrink-0 z-10 backdrop-blur">
            <span className="text-sm font-medium text-muted-foreground">
              <strong className="text-white">{selected.size}</strong> {selected.size === 1 ? 'repository' : 'repositories'} selected
            </span>
            <Button 
              onClick={handleContinue} 
              disabled={selected.size === 0 || saving}
              className="bg-blue-600 hover:bg-blue-700 text-white px-8 h-10 font-semibold shadow-[0_0_20px_rgba(37,99,235,0.4)] disabled:opacity-50"
            >
